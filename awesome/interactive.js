var Interactive = {
    ...
    init: function(graph, mouselayer) {
        Interactive.graph = graph;
        Interactive.mouselayer = mouselayer;
        $(graph.canvas).css("position", "absolute");
        $(mouselayer.canvas).css("position", "absolute");
        $(mouselayer.canvas).css("z-index", 1);
    },
...
*/
    ...
    findAngle: function(point1, point2, vertex) {
        if (vertex === undefined) {
            ...
            return Interactive.findAngle(point1, vertex) - Interactive.findAngle(point2, vertex);
        }
    },

    // Draw angle arcs
    drawArcs: function(point1, vertex, point3, numArcs) {
        var startAngle = Interactive.findAngle(point1, vertex);
        var endAngle = Interactive.findAngle(point3, vertex);
    ...
    addMovablePoint: function(options) {
        ...
        var graph = Interactive.graph;

        if (movablePoint.visible) {
            movablePoint.visibleShape = graph.circle(movablePoint.coord[0], movablePoint.coord[1], 4).attr(movablePoint.normalStyle);
        }
        ...
        movablePoint.applyConstraint = function(coord, extraConstraints, override) {
            ...
            // both distance and angle are constrained
                ...
                var constrainedAngle = (constraints.fixedAngle.angle + Interactive.findAngle(ref, vertex)) * Math.PI / 180;
            ...
            // angle is constrained
                ...
                var angle = Interactive.findAngle(coord, vertex) * Math.PI / 180;
            ...
            // distance is constrained
                ...
                var angle = Interactive.findAngle(coord, distPoint);
                var length = constraints.fixedDistance.dist;
        ...
        if (movablePoint.visible && !movablePoint.constraints.fixed) {
            ...
            movablePoint.mouseTarget = Interactive.mouselayer.circle(movablePoint.coord[0], movablePoint.coord[1], 15);
            ...
            $(movablePoint.mouseTarget[0]).bind("vmousedown vmouseover vmouseout", function(event) {
                if (event.type === "vmouseover") {
                    ...
                    $(document).bind("vmousemove vmouseup", function(event) {
                        ...
                        var mouseX = event.pageX - $(graph.canvas.parentNode).offset().left;
                        var mouseY = event.pageY - $(graph.canvas.parentNode).offset().top;
                        // can't go beyond 10 pixels from the edge
                        mouseX = Math.max(10, Math.min(graph.width - 10, mouseX));
                        mouseY = Math.max(10, Math.min(graph.height - 10, mouseY));

                        // snap to grid
                        if (movablePoint.snapX) {
                            mouseX = Math.round(mouseX / movablePoint.snapX) * movablePoint.snapX;
                        }
                        if (movablePoint.snapY) {
                            mouseY = Math.round(mouseY / movablePoint.snapY) * movablePoint.snapY;
                        }
                        // snap mouse to grid
        ...
        movablePoint.setCoord = function(coord) {
            if (this.visible) {
                var scaledPoint = coord.slice();//graph.scalePoint(coord);
    ...
    addInteractiveFn: function(fn, options) {
        ...
        interactiveFn.mouseTarget = Interactive.mouselayer.path(this.svgPath(points));
    ...
    addMovableLineSegment: function(options) {
        ...
        var path = Interactive.svgPath([[0, 0], [1, 0]]);
        /*
        for (var i = 0; i < lineSegment.ticks; ++i) {
        ...
        */
        lineSegment.visibleLine = graph.path(path);
        lineSegment.visibleLine.attr(lineSegment.normalStyle);

        if (!lineSegment.fixed) {
            lineSegment.mouseTarget = Interactive.mouselayer.rect(0, -15, 1, 30);
        ...
        lineSegment.transform = function(syncToPoints) {
            ...
            var angle = Interactive.findAngle(this.coordZ, this.coordA);
            var scaledA = this.coordA;
            var lineLength = Interactive.getDistance(this.coordA, this.coordZ);
            if (this.extendLine) {
                if (this.coordA[0] !== this.coordZ[0]) {
                    ...
                    if (this.coordA[0] < this.coordZ[0]) {
                        scaledA = [graph.range[0][0], y1];
                        scaledA[0]++;
                    } else {
                        scaledA = [graph.range[0][1], y2];
                        scaledA[0]--;
                    }
                    lineLength = KhanUtil.getDistance([graph.range[0][0], y1], [graph.range[0][1], y2]);
                } else {
                    if (this.coordA[1] < this.coordZ[1]) {
                        scaledA = [this.coordA[0], graph.range[1][0]];
                    } else {
                        scaledA = [this.coordA[0], graph.range[1][1]];
                    }
        ...
        if (!lineSegment.fixed && !lineSegment.constraints.fixed) {
            ...
            $(lineSegment.mouseTarget[0]).bind("vmousedown vmouseover vmouseout", function(event) {
                if (event.type === "vmouseover") {
                    ...
                    //var coordX = (event.pageX - $(graph.canvas.parentNode).offset().left);
                    //var coordY = -(event.pageY - $(graph.canvas.parentNode).offset().top);
                    ...
                    var offsetLeft = -Math.min(mouseOffsetA[0], mouseOffsetZ[0]);
                    var offsetRight = Math.max(mouseOffsetA[0], mouseOffsetZ[0]);
                    var offsetTop = Math.max(mouseOffsetA[1], mouseOffsetZ[1]);
                    var offsetBottom = -Math.min(mouseOffsetA[1], mouseOffsetZ[1]);
    ...
    addArrowWidget: function(options) {
        ...
        arrowWidget.mouseTarget = Interactive.mouselayer.circle(
